...
float temp[7];
float x, x_p, y, y_p, z, P, Px, Py, Pz, Pt2, Ptest;
...
void textfile()
{
	...
	{	
		count++;
		for(int i=0; i<7; i++) TurtleInput>>temp[i];
		...
		out_txt<<x<<setw(12)<<y<<setw(12)<<z<<setw(12)<<Px<<setw(12);
