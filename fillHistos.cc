...
void fillHistos()
{
	...
	nentries0 = (Int)tree0->GetEntries();
	...
	for(long counter=0; counter<nentries0; counter++)
	{
		...
		if(counter%1000 == 0 && counter>0) cerr<< " entry number = "<<(nentries0 - counter)<<",  event number = "<<Det0_EventID<<endl<<endl;
	...
	nentries3 = (Int)tree3->GetEntries();
	...
	for(long counter3=0; counter3<nentries3; counter3++)
	{
		...
		if(counter3%1000 == 0 && counter3>0) cerr<< " entry number = "<<(nentries3 - counter3)<<",  event number = "<<Det3_EventID<<endl<<endl;
	...
	nentriesTar = (Int)treeTar->GetEntries();
	...
	for(long counterTar=0; counterTar<nentriesTar; counterTar++)
	{
		...
		if(counterTar%1000 == 0 && counterTar>0) cerr<< " entry number = "<<(nentriesTar - counterTar)<<",  event number = "<<DetTar_EventID<<endl<<endl;
